...
TITLE = "Caltech101 (cnn)"
OUTPUT_DIR = "output/cnn_caltech101"

TARGET_VALIDATION_ACC = 0.5
TARGET_TRAIN_ACC = 0.5
TARGET_F1 = 0.4
...
def get_task_metadata():
    ...
    return {
        "task_name": "cnn_caltech101",
        "task_type": "neural",
        "input_type": "image",
        "output_type": "classification",
        "description": "Fit a convolutional neural network on Caltech101 using ADAM."
    }
...
def make_dataloaders(batch_size=32, device=None):
    ...
    # train_dataset = torchvision.datasets.Imagenette(".data/imagenette", split="train", download=True, transform=transform)
    dataset = torchvision.datasets.Caltech101(".data/caltech101", target_type="category", download=True, transform=transform)
    a, b = dataset[0]
...
def train(model, train_loader, val_loader, device=None, epochs=100, lr=0.1, weight_decay=0.01):
    ...
    for epoch in range(epochs):
        ...
        # eprint progress every 20 epochs
...
def save_artifacts(model, train_losses, val_losses, output_dir="output", filename_prefix="nn_mnist"):
    ...
    eprint(f"Artifacts saved to {output_dir}")
...
def main():
    ...
    train_loader, val_loader= make_dataloaders(batch_size=BATCH_SIZE, device=device)
    eprint(f"Training samples: {len(train_loader) * BATCH_SIZE}, Validation samples: {len(val_loader) * BATCH_SIZE}")
    
    # Build model
    eprint("\nBuilding model...")
    output_dimension = 101
    model = Model(output_dimension).to(device)
    eprint(f"Model architecture: {model}")
    ...
    eprint("\nMetrics:")
    eprint(f"  Train - Loss: {train_metrics['loss']:.4f}, F1: {train_metrics['f1_macro']:.4f}, Accuracy: {100*train_metrics['accuracy']:.2f}%")
    eprint(f"  Val   - Loss: {val_metrics['loss']:.4f}, F1: {val_metrics['f1_macro']:.4f}, Accuracy: {100*val_metrics['accuracy']:.2f}%")
    ...
    # Quality checks
    eprint("\n" + "=" * 60)
    eprint("Quality Checks:")
    eprint("=" * 60)
    
    checks_passed = True

    if train_metrics['accuracy'] > TARGET_TRAIN_ACC:
        eprint(f"✓ Train accuracy > {TARGET_TRAIN_ACC}: {train_metrics['accuracy']:.4f}")
    else:
        eprint(f"✗ Train accuracy > {TARGET_TRAIN_ACC}: {train_metrics['accuracy']:.4f}")
        checks_passed = False
    
    if val_metrics['accuracy'] > TARGET_VALIDATION_ACC:
        eprint(f"✓ Val accuracy > {TARGET_VALIDATION_ACC}: {val_metrics['accuracy']:.4f}")
    else:
        eprint(f"✗ Val accuracy > {TARGET_VALIDATION_ACC}: {val_metrics['accuracy']:.4f}")
        checks_passed = False
    ...
    eprint("=" * 60)
    if checks_passed:
        eprint("PASS: All quality checks passed!")
    else:
        eprint("FAIL: Some quality checks failed!")
    eprint("=" * 60)
